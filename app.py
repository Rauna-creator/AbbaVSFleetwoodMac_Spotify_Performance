...
try:
    ...
    if page == "Overview":
        ...
        st.markdown("---")
        st.subheader("Feature Comparison Visualizations")
        
        # Create 2x3 grid of boxplots
                ...
                if i + j < len(features):
                    ...
                    with col:
                        fig = px.box(pd.concat([
                            hits[[feature]].assign(Category='Hits'),
                            deep_cuts[[feature]].assign(Category='Deep Cuts')
                        ]), x='Category', y=feature, color='Category',
                        title=feature,
